"""
Strategy 2: Direct Solver (MUMPS/SuperLU)
"""
import jax
...
class StandardDruckerPrager(Problem):
...
def run_test(displacement):
    Lx, Ly, Lz = 10., 10., 10.
    Nx, Ny, Nz = 2, 2, 2
    data_dir = os.path.join(project_root, 'results', 'test_output_dp_s2')
    ...
    problem = StandardDruckerPrager(mesh, vec=3, dim=3, dirichlet_bc_info=dirichlet_bc_info)
    
    # Direct Solver Options (Attempting MUMPS)
    solver_options = {
        'petsc_solver': {
            'ksp_type': 'preonly', 
            'pc_type': 'lu',
            'pc_factor_mat_solver_type': 'mumps' # Key change
        }
    }
...
if __name__ == "__main__":
    ...
    print("Strategy 2: Direct Solver (MUMPS)")
    print("="*40)
