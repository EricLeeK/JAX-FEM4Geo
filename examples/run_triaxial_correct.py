"""
Corrected Triaxial Compression Test Simulation

Improvements:
1. Direct Solver (MUMPS/LU) to handle potential singularity
2. Higher mesh density
3. Explicit Stage 1 (Iso) and Stage 2 (Shear)
"""
...
class StandardTriaxialProblem(Problem):
    ...
    def get_tensor_map(self):
        ...
        def stress_return_map(u_grad, sigma_old, epsilon_old):
            ...
            eps_inc = 0.5 * (u_grad + u_grad.T) - epsilon_old
            ...
            d_lam = f_plus / (1. + 3.*alpha*alpha)
            return sig_trial - d_lam * (s_dev / np.sqrt(J2 + a*a) + alpha * np.eye(self.dim))
...
def run():
    sigma3 = 50.0
    results_dir = os.path.join(project_root, 'results', 'triaxial_correct')
    os.makedirs(results_dir, exist_ok=True)
    
    meshio_mesh = box_mesh_gmsh(Nx=2, Ny=2, Nz=2, domain_x=10, domain_y=10, domain_z=10, data_dir=results_dir, ele_type='HEX8')
    ...
    # Use Direct Solver to avoid PETSc convergence issues
    solver_options = {'petsc_solver': {'ksp_type': 'preonly', 'pc_type': 'lu', 'pc_factor_mat_solver_type': 'mumps'}}

    dir_iso = [[bottom, bottom, bottom], [0, 1, 2], [lambda p:0., lambda p:0., lambda p:0.]]
    ...
    for i in range(15):
        d = (i+1)/15 * (-0.3) # 3% strain
        new_dir = [[bottom, bottom, bottom, top_loc], [0, 1, 2, 2], [lambda p:0., lambda p:0., lambda p:0., lambda p,d=d: d]]
    ...
    plt.plot([sigma3, max(p_h)], [0, 3*(max(p_h)-sigma3)], 'b--', label='Theoretical (Slope=3)')
    plt.xlabel('Mean Stress p [MPa]'); plt.ylabel('Deviatoric Stress q [MPa]')
    plt.title('Standard Triaxial Stress Path (Iso -> Shear)'); plt.legend(); plt.grid()
    plt.savefig(os.path.join(results_dir, 'standard_triaxial_path.png'))
    print(f"Graph saved in {results_dir}")
